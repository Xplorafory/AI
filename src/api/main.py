...
def check_auth(authorization: str = Header(default="")):
    """Simple bearer check using RAG_SHARED_SECRET. Keep /healthz public."""
...
def _init_openai():
    ...
    if OPENAI_API_KEY:
        try:
            ...
            print(f":warning: OpenAI init failed → falling back to local embeddings: {e}", file=sys.stderr)
            use_local = True
    else:
        use_local = True
...
def embed(text: str) -> List[float]:
    """Return a numeric vector (list[float]) for the query."""
    if not use_local:
        if client is None:
            _init_openai()
        if client:
...
def guess_table(conn) -> str:
    ...
    if use_local:
        ...
        if "3-large" in EMB_MODEL:
            desired = desired or "kb_chunks"          # 3072
        elif "3-small" in EMB_MODEL:
            desired = desired or "kb_chunks_1536"     # 1536
        else:
...
def search(conn, table: str, qvec: List[float], k: int = 6) -> List[Dict[str, Any]]:
    """Use %s::vector with string literal '[...]' to avoid casting errors."""
